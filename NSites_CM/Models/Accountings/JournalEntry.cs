...
namespace NSites_CM.Models.Accountings
{
    public class JournalEntry
    {
            ...
            string pSupplierId, string pCustomerId, string pUserId, string pRemarks)
        {
            ...
            {
                _conn.Open();
                string _begBal = "N", _adjustment = "N", _closingEntry = "N", _SOId = "", _POId = "";
                MySqlTransaction _trans = _conn.BeginTransaction();
                MySqlCommand _cmd = new MySqlCommand("call spInsertJournalEntry('" + pFinancialYear +
                    "','" + pJournal +
                    "','" + pForm +
                    "','" + pVoucherNo +
                    "','" + String.Format("{0:yyyy-MM-dd}", pDatePrepared) +
                    "','" + pExplanation +
                    "','" + pTotalDebit +
                    "','" + pTotalCredit +
                    "','" + pReference +
